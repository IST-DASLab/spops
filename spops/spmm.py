...
def spmm(A_val, A_row_offsets, A_row_indices, A_col_indices, B, M, backend=None, force_fp32=True, return_type=None):
    ...
    if A_val.is_cuda:
        ...
        if run_full_fp16:
            out = cuda_backend.sputnik_spmm_fp16(A_val, A_row_offsets.to(int32), A_row_indices.to(int32), A_col_indices.to(int16), B, M)[0]
        else:
            out = cuda_backend.sputnik_spmm_fp32(A_val.to(float32), A_row_offsets.to(int32), A_row_indices.to(int32), A_col_indices.to(int32), B.to(float32), M)[0]
    else:
        ...
        getattr(cpu_backend, backend)(A_val, A_row_offsets, A_col_indices, B, out)
