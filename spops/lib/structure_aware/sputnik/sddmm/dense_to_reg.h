...
struct DenseToReg {
  ...
  typedef typename Value2SharedIndex<LoadType>::Index IndexType;
  ...
  const INDEX_TYPE *column_indices_;
  ...
  INDEX_TYPE *column_idxs_tile_base_;
  ...
  __device__ __forceinline__ DenseToReg(
      int k, int row_offset, int column_offset, const INDEX_TYPE *column_indices,
      const float *matrix, INDEX_TYPE *column_indices_tile, float *matrix_fragment)
...
#pragma unroll
  ...
  __device__ __forceinline__ void Load() {
    ...
    asm volatile("" ::: "memory");
#endif
    INDEX_TYPE *column_idxs_tile = column_idxs_tile_base_;
#pragma unroll
    for (int x_item_idx = 0; x_item_idx < kThreadItemsX_; ++x_item_idx) {
      // Load the column index and offset the matrix pointer.
    	// We multiply kDimK_ after the fact since it's now short
      const LoadType *matrix =
          OffsetCast<const LoadType>(matrix_base_, kDimK_ * (*column_idxs_tile));
#pragma unroll
