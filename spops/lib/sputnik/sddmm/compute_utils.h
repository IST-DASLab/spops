...
struct ComputeUtilsSDDMM {
  ...
  __device__ __forceinline__ ComputeUtilsSDDMM(const float* lhs_fragment_,
...
#pragma unroll
      for (int x_item_idx = 0; x_item_idx < kThreadItemsX; ++x_item_idx) {
        const float rhs_value = rhs_fragment[k_item_idx + x_item_idx * kThreadItemsK];
